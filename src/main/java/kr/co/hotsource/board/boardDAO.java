...
public class boardDAO {
	...
	public int create(boardDTO dto) {
		...
		try {
			...
			sql.append(" VALUES ((SELECT IFNULL(MAX(boardno),0)+1 FROM board as ICB),?, ?, ?, 'F', ?, ?) ");
	...
	public List list(int start, int end, String strSearchtype, String strSearch) throws Exception{
	    ...
	    try {
	      ...
	      sql.append("       FROM ( ");
	      sql.append("             SELECT boardno,title,content,id,date,readcnt,good,replycnt ");
	      sql.append("             FROM board, (SELECT @rownum:=0)R ");
	      sql.append("             WHERE code='F' ORDER BY boardno ASC ");
	      sql.append("            ) b ");
	      sql.append("       ) a ");
	...
	public int getArticleCount() throws Exception {
		...
		try {
			con = dbopen.getConnection();
			pstmt = con.prepareStatement("SELECT count(*) FROM board WHERE code='F'");
			rs = pstmt.executeQuery();
			if (rs.next()) {
				res = rs.getInt(1);
			}
	...
	public int replycntdown(int boardno) {
		...
		try {
			...
			System.out.println("댓글수 감소 실패 : " + e);
		} finally {
			dbclose.close(con, pstmt);
		}
		return res;
	...
	public List best() throws Exception {
		...
		try {
			...
			sql.append(" SELECT boardno, title, id, date, readcnt, good ");
			sql.append(" FROM board ");
			sql.append(" ORDER BY readcnt DESC, good DESC limit 5 ");
